...
3. Open a git folder from File or the workspace control. Kronos indexes it locally. Chat can search, read, write, and run commands inside that folder. AGENTS.md, .cursorrules, and files under .cursor/rules are followed on every turn. Apply on a code block writes that file into the folder. Files lets you edit a text file. Save or Ctrl+S writes it into the folder. Ctrl+P or File → Go to file jumps to a path. Ctrl+F finds text in the open file. Ctrl+H replaces it. Common languages are colored. Click a file mention in chat to open it in Files. Paste a screenshot into chat to ask about the UI. View → Terminal opens a shell in that folder. Type a command and press Enter. Output stays as you send more lines. Stop or Escape in the command box ends the shell.
