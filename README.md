...
3. Open a git folder from File or the workspace control. Kronos indexes it locally. Chat can search, read, write, and run commands inside that folder. AGENTS.md, .cursorrules, and files under .cursor/rules are followed on every turn. Apply on a code block writes that file into the folder. Files lets you edit a text file. Save or Ctrl+S writes it into the folder. Ctrl+P or File → Go to file jumps to a path. Ctrl+F finds text in the open file. Match case limits that search to the same letter case. Whole word and Regular expression refine that search. Ctrl+Shift+F searches the workspace index. Click a hit to open that line. Ctrl+H replaces text in the open file. Ctrl+G jumps to a line. Common languages are colored. Click a file mention in chat to open it in Files. Paste a screenshot into chat to ask about the UI. View → Terminal opens a shell in that folder. Type in the panel. Stop ends the shell.
4. Changes from the git working tree show in the right inspector. This turn lists files chat wrote. All lists every dirty file. Open a change to see added and removed lines. Open edits the file in Files. Revert restores the last chat write, or the last committed version if chat did not write that file. Commit records a local git commit of the visible list. Kronos does not push. Health ticks cover engine, model, workspace, index, and secrets.
