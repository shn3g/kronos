...
4. Changes from the git working tree show in the right inspector. This turn lists files chat wrote. All lists every dirty file. Open edits the file in Files. Revert restores the last chat write, or the last committed version if chat did not write that file. Commit records a local git commit of the visible list. Kronos does not push. Health ticks cover engine, model, workspace, index, and secrets.
