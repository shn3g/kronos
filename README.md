...
3. Open a git folder from File or the workspace control. Kronos indexes it locally. Chat can search, read, write, and run commands inside that folder. AGENTS.md, .cursorrules, and files under .cursor/rules are followed on every turn. Apply on a code block writes that file into the folder. Click a file mention in chat to open it in Files. Paste a screenshot into chat to ask about the UI. View → Terminal runs a command you type. Output appears as it runs. Stop, Escape, or Ctrl+C ends a running command.
4. Changes from the git working tree show in the right inspector. This turn lists files chat wrote. All lists every dirty file. Open reads the file in Files. Revert restores the last chat write, or the last committed version if chat did not write that file. Commit records a local git commit of the visible list. Kronos does not push. Health ticks cover engine, model, workspace, index, and secrets.
