...
"""Replay klikday decisions against Kronos observe/shadow outcomes. No I/O."""
...
def kronos_shadow_outcome(
    ...
    *,
    ...
    action = _KLIKDAY_ACTION.get(klikday.action, klikday.action)
    wrote = False
    identity = "none"
    try:
        refuse_mode_write(
            mode,
            action,
            target_branch=klikday.target_branch,
            protected_branch=protected_branch,
        )
        if klikday.identity in {"comment", "label"} and action == "merge_integration":
            identity = klikday.identity
        elif klikday.target_branch == protected_branch:
            identity = "app_check"
        else:
            identity = "app_check"
        ...
        identity = "none"
    return ReplayEvent(
        task_id=klikday.task_id,
        source="kronos",
        action=klikday.action,
        target_branch=klikday.target_branch,
        identity=identity,
        payload="",
        wrote=wrote,
