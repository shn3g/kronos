...
def test_klikday_lessons_import_as_disabled_candidates(tmp_path: Path) -> None:
    catalog = _catalog(tmp_path, {})
    imported = catalog.procedural.import_klikday_lessons(klikday_lessons_yaml())
...
def test_retrieval_uses_text_and_has_no_booking_or_a11y_boosts(tmp_path: Path) -> None:
    ...
    catalog.procedural.import_klikday_lessons(klikday_lessons_yaml())
...
async def test_skills_and_memory_http_surfaces(tmp_path: Path) -> None:
    ...
    async with AsyncClient(
        ...
        lessons = await http.post(
            ...
            headers=headers,
            json={"yaml": klikday_lessons_yaml()},
        )
