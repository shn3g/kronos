...
"""Replay klikday dispatch/merge fixtures against Kronos observe/shadow outcomes."""
...
def test_shadow_refuses_writes_that_klikday_would_perform() -> None:
    dispatch = _klikday(action="open_draft_pr", identity="none")
    merge_main = _klikday(action="merge", target_branch="main", identity="comment")
    shadowed = (
