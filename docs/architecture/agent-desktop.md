...
- Files edits one text file at a time. There is no tabbed IDE. Line numbers sit beside the text. Common languages are colored. Ctrl+F finds in the open file. Match case limits that search to the same letter case. Whole word and Regular expression refine that search. Ctrl+Shift+F searches the workspace index. Click a hit to open that line. Ctrl+H replaces in the open file. Ctrl+G jumps to a line.
...
- Review on the right: Changes is the default inspector tab (Cursor diffs, VS Code Changes panel). It lists the live git working tree. This turn shows files chat wrote and has not committed. All shows every dirty file. Revert restores the last chat write, or HEAD if chat did not write that file. Open a change to see added and removed lines. Commit records a local git commit of the visible list and does not push. Open reads the file in Files for editing. Save or Ctrl+S writes it through the same workspace write path as chat Apply. Ctrl+P or File → Go to file jumps to a path without walking the tree. Ctrl+F finds text in the open file. Match case limits that search to the same letter case. Whole word and Regular expression refine that search. Ctrl+H replaces it. Ctrl+G jumps to a line. Line numbers sit beside the editor. Common languages are colored. Ask in chat mentions the selected file. Click a file mention in chat, or an inline path in a reply, to open it in Files. Ctrl+Shift+F searches the local index. Click a hit to open that line. The composer shows an estimated token count against a 32,000 token window. At 80 percent it asks you to start a new chat. Terminal is a View toggle and Ctrl+` panel. It opens a real TTY in the current workspace folder and does not pass engine secrets into that process. Type in the panel. Stop ends the shell.
