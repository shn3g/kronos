...
- Review on the right: Changes is the default inspector tab (Cursor diffs, VS Code Changes panel). It lists the live git working tree. Revert restores the last chat write, or HEAD if chat did not write that file. Commit records a local git commit and does not push. Files tree, last-turn filter, terminal, and context meter stay later.
