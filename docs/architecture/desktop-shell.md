...
The Tauri 2 client in `apps/desktop` locates a loopback engine through `engine_connection` and probes `/health` plus `/version`. Until 0.5.0 the engine on PATH must match the desktop version; a newer desktop against an older engine is **incompatible**. Without a sidecar (Vite `preview` / Playwright smoke), production wiring stays **engine unavailable**.
...
On Linux CI, after smoke passes on a ready-for-review pull request, `pnpm test:e2e:engine` (`KRONOS_E2E_ENGINE=1`) runs the **with-engine** project: a real `python3 -m kronos_engine` sidecar and a mock OpenAI-compatible model. That exercises Files, Changes, Terminal, and Goals workbench in the browser build. Native artifacts still build in the `desktop` CI job on Ubuntu, Windows, and macOS.
