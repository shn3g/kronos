...
Kronos is the documented source of truth for Klikday automation going forward. Public GitHub visibility for `shn3g/kronos` is a controller step after this work is approved and merged. Do not flip visibility from a feature branch.

## Staged cutover

1. Commit `examples/klikday/config.yaml` as `.kronos/config.yaml` on `main-openclaw` (shadow or observe).
2. Import `examples/klikday/lessons.yaml` as disabled candidates.
...
5. Keep klikday `scripts/agent-ops` and merge-gate in place as the operator fallback.

## Rollback

Freeze Kronos autonomy and leave klikday wrappers as fallback. Do not re-enable write crons from this procedure. The contain change that paused write-capable wrappers stays unmerged until its own review.
...
4. Use klikday wrappers only as a manual operator fallback. Do not turn write crons back on.

Tests: `engine/tests/unit/application/test_migration_rollback.py`.

## Removing embedded automation

Delete `scripts/agent-ops` and related klikday factory crons only after **two stable Kronos release cycles** with:
...
That cleanup is a dedicated Klikday PR. Do not mass-delete wrappers in the shadow-config change.
