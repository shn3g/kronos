...
Enrolment can propose a ruleset. It must require the reviewer check with `integration_id`, keep strict required status checks, and zero bypass actors. Kronos will not copy klikday's `strict_required_status_checks_policy: false`.
...
Klikday merge-gate and hermes-reviewer identity stay in that product repository until a dedicated cleanup after two stable Kronos release cycles. Kronos does not use Hermes check names.
