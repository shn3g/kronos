...
The version Cursor and the desktop app read is `0.3.0` in `apps/desktop/src-tauri/tauri.conf.json`, kept in lockstep with `package.json`, `apps/desktop/src-tauri/Cargo.toml`, `engine/pyproject.toml`, and `services/reviewer/pyproject.toml`. `scripts/check-version-sync.py` fails CI when those files disagree.
